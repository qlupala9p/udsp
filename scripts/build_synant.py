#!/usr/bin/env python3
"""One-off generator for data/synanten.js and data/synantde.js.
...
`definition`/`example` are intentionally left as empty strings for every
entry: hand-authoring accurate definitions/examples for ~16,000 words is not
feasible in one pass, and Word Morph's gameplay only needs word/synonyms/
antonyms. This is a deliberate, documented scope decision (see repo memory).
"""
import os
import re
import tempfile
...
def write_js(path, varname, entries, source_note):
    ...
    lines.append(
        "// intentionally empty (hand-authoring ~%d entries is not feasible); synonyms/antonyms are"
        % len(entries)
    )
    lines.append("// semicolon-separated. Used ONLY by Word Morph (wordmorph.js), independent of WORD_SETS.")
