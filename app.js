...
(function () {
  ...
  function startCloze(level) {
    ...
    if (s) s.hidden = true;
    if (g) g.hidden = false;
    $("cloze-level-badge").textContent = levelLabel(level);
    $("cloze-wins").textContent = czWins;
    $("cloze-losses").textContent = czLosses;
    newClozeItem();
  }
  ...
  function renderClozeItem() {
    var it = czItem;
    $("cloze-pos").textContent = it.entry.pos || "";
    ...
    $("cloze-feedback").textContent = "";
    $("cloze-feedback").className = "feedback";
  ...
  function answerCloze(idx, btn) {
    ...
    var optionEls = $("cloze-options").querySelectorAll(".option");
  ...
  function showClozeResult(isCorrect) {
    ...
    $("cloze-answer").innerHTML =
      '<span class="hm-answer-label">Word:</span> <strong>' +
      escapeHtml(w.word) +
      "</strong>";
    var full = $("cloze-full-sentence");
    full.textContent =
  ...
  (function wireCloze() {
    var back = $("cloze-back");
    var change = $("cloze-change");
    var next = $("cloze-next");
    if (back) back.addEventListener("click", showClozeSetup);
    if (change) change.addEventListener("click", showClozeSetup);
    if (next) next.addEventListener("click", newClozeItem);
    document.addEventListener("keydown", function (e) {
      ...
      if (k in map) {
        var idx = map[k];
        var btns = $("cloze-options").querySelectorAll(".option");
